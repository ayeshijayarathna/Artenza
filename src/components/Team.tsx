const team = [
  {
    ...
    bio: "With over 15 years in fine art curation, Elena brings an eye for emerging talent and a passion for跨界 expression.",
  },
  {
    name: "James Okonkwo",
    role: "Lead Curator",
    ...
    <section id="team" className="py-24 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {team.map((member) => (
            <div key={member.name}
              className="bg-charcoal/60 border border-warm-gray/20 rounded-lg p-6 text-center hover:border-gold/30 transition-colors">
              <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gold/10 flex items-center justify-center">
                <span className="text-gold text-2xl font-serif">{member.name[0]}</span>
              </div>
              <h3 className="font-serif text-lg font-semibold text-linen mb-1">{member.name}</h3>
              <p className="text-gold text-xs tracking-wider uppercase mb-3">{member.role}</p>
